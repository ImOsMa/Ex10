...
std::string infix2postfix(std::string expr) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (!priority(expr[i]) && (expr[i] >= 48 && expr[i] <= 57 || expr[i] == '.')) {
      changeExpr += expr[i];
      if ((!priority(expr[i + 1]) || expr[i + 1]) && (expr[i + 1] < 48 || expr[i + 1] > 57) && expr[i + 1] != '.') {
        changeExpr += ' ';
      }
      continue;
    }
      ...
      opStack.pop();  
    } else if (priority(opStack.get()) < priority(expr[i]) && !opStack.isEmpty()) {
      opStack.push(expr[i]);
      ...
      while (priority(expr[i]) <= priority(opStack.get()) && opStack.isEmpty() == 0) {
